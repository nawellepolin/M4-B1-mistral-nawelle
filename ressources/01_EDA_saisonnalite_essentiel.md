...
- **Distribution conditionnelle** : `sns.boxplot(data=df, x="season", y="total_rentals")`
  → comment la cible varie selon une feature catégorielle.
- **Profil moyen** : `df.groupby("hour")["total_rentals"].mean().plot()` → forme
  typique de la journée (pic matin + soir, creux nuit).
- **Heatmap** : `df.groupby(["weekday", "hour"])["total_rentals"].mean()` puis pivot
...
- **Corrélation forte** température ↔ total_rentals (positive)
